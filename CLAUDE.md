...
| `compute/` | 数据计算（k1..k3, k5；k4/k6/k7 预留） | `kN_<desc>.py` 命名 |
| `strategy/` | 策略（含 `plugins/` 放单标的插件；`cross_section/` 预留） | `base/registry/context/risk/selector` 是骨架；`plugins/pNN_<desc>.py` 是单标的插件 |
...
| `tests/` | 验证 | `test_<desc>.py` 命名，下划线开头的标记文件进 `_deprecated/legacy_tests/` |
...
| 目录 | 作用 |
|---|---|
| `data/market_data/` | 板块个股映射（新版数据源），被 `lib.relation_graph` 和 `collect/c5_mapping` 引用 |
| `data/snapshots/` | 临时离线快照（备用） |
| `data/refresh_log/` | 刷新日志（增量同步状态） |
