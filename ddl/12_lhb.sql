...
-- 营业部识别: config/broker_list.py 的 FAMOUS_BROKERS / BROKER_LABELS
...
-- 数据源: K:\QTM\longhubang.db3 的 longhubang + trader_booth 表
-- 时间戳: lhb_date (龙虎榜日期)
-- 字段映射:
--   code         ← stock_code
--   lhb_date     ← trade_date
--   stock_name   ← stock_name (股票名称, 新增)
--   close_price  ← close_price (收盘价, 新增)
--   rise_and_fall ← rise_and_fall (涨跌幅, 新增)
--   reason       ← reason (上榜原因)
--   reason_id    ← reason_id (上榜原因代码, 新增)
--   direction    ← direction (B/S 买卖方向, 新增)
--   rank         ← rank (营业部排名)
--   buy_amount   ← buyin_amount (买入金额)
--   buyin_ratio  ← buyin_ratio (买入占比, 新增)
--   sell_amount  ← sellout_amount (卖出金额)
--   sellout_ratio ← sellout_ratio (卖出占比, 新增)
--   net_amount   ← net_amount (净额)
--   broker_name  ← business_department (营业部名称)
--   broker_type  ← FAMOUS_BROKERS 识别 (hot_money/institution/north)
--   broker_label ← BROKER_LABELS 标签 (拉萨天团/杭州游资/机构 等)
